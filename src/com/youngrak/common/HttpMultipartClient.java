 ...
 * 
 * Know cornercases:
 * If server reply has no body (content-lenght = 0) and connection type is
 *  keep-alive, the client remains connected and freezed, until the server
 *  close it.
 *  
 *  
...
public class HttpMultipartClient {
    ...
    // ---------- Private fields
    private static final String TAG = HttpMultipartClient.class.getSimpleName();

    private static final int CONNECTION_TIMEOUT = 10000;
    private static final String END = "\r\n";
    private final String boundary = Integer.valueOf(
    ...
    public interface OnProgressListener{
    	void onFileProgressChanged(int progress);
    }
    ...
    public void send() throws IOException {
        ...
        try {
            ...
            while ((size = fileStream.read(bytes)) > 0) {
                ...
                int progress = (int)(( bytesSent / (float)length) * 100);
                //Log.d(TAG, "onProgressListener progress="+progress+" bytesSent="+bytesSent+" length="+length);
                if(onProgressListener != null){
                	//Log.d(TAG, "onProgressListener progress");
                	onProgressListener.onFileProgressChanged( progress);
                }
            }
            ...
            Log.d("responseBody", "ready");
            StringBuffer payload = new StringBuffer();
            Log.e("responseBody", "ready1");
            boolean bodyEnd = false;
            Log.e("responseBody", "ready2");
